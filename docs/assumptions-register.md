...
| # | Question | Basis | Severity | Owner | Needed by |
|---|---|---|---|---|---|
| Q-27 | ~~Rate limiting on the anonymous qualifier endpoints.~~ **Closed in the CC-02 hardening pass.** Fixed-window limiting per client address and action, keyed on a daily-rotating HMAC so no raw address is stored; refusal is an ordinary accessible page with a route to a person, never a challenge. Two things still need a human decision: (a) confirm the limit values, which were sized to absorb a shared office NAT rather than a single user; (b) the deployment constraint that the origin is only reachable through the proxy, since `x-forwarded-for` is otherwise spoofable. | §16 Assurance, ACC-005 | B2 | Security/privacy lead | CC-09 |
